    ...
    def drawGeneBand(self, canvas, gifmap, plotXScale, offset= (40, 120, 80, 10), zoom = 1, startMb = None, endMb = None):
        ...
        for gIndex, theGO in enumerate(self.geneCol):
            ...
            if self.endMb - self.startMb <= self.DRAW_DETAIL_MB and geneEndPix - geneStartPix > self.EACH_GENE_ARROW_SPACING * 3:
                ...
                im_drawer.line(
                    xy=((geneStartPix, geneYLocation + self.EACH_GENE_HEIGHT/2*zoom),( geneEndPix, geneYLocation + self.EACH_GENE_HEIGHT/2*zoom)),
                ...
                for xCoord in range(0, genePixRange):
                    ...
                    if (xCoord % self.EACH_GENE_ARROW_SPACING == 0 and xCoord + self.EACH_GENE_ARROW_SPACING < geneEndPix-geneStartPix) or xCoord == 0:
                        if strand == "+":
                            im_drawer.line(
                                xy=((geneStartPix + xCoord, geneYLocation),( geneStartPix + xCoord + self.EACH_GENE_ARROW_WIDTH, geneYLocation +(self.EACH_GENE_HEIGHT / 2)*zoom)),
                                fill=arrowColor, width=1)
                            im_drawer.line(
                                xy=((geneStartPix + xCoord, geneYLocation + self.EACH_GENE_HEIGHT*zoom),( geneStartPix + xCoord+self.EACH_GENE_ARROW_WIDTH, geneYLocation + (self.EACH_GENE_HEIGHT / 2) * zoom)),
                                fill=arrowColor, width=1)
                        else:
                            im_drawer.line(
                                xy=((geneStartPix + xCoord + self.EACH_GENE_ARROW_WIDTH, geneYLocation),( geneStartPix + xCoord, geneYLocation +(self.EACH_GENE_HEIGHT / 2)*zoom)),
                                fill=arrowColor, width=1)
                            im_drawer.line(
                                xy=((geneStartPix + xCoord + self.EACH_GENE_ARROW_WIDTH, geneYLocation + self.EACH_GENE_HEIGHT*zoom),( geneStartPix + xCoord, geneYLocation + (self.EACH_GENE_HEIGHT / 2)*zoom)),
                                fill=arrowColor, width=1)
                ...
                if self.genotype[0][i].name != " - " :
                    ...
                    for j,_geno in enumerate (self.genotype[0][i].genotype):
                        ...
                        if (plotbxd == 1):
                            ...
                            im_drawer.line(
                                xy=((drawStart, geneYLocation+7+2*ind*self.EACH_GENE_HEIGHT*zoom),( drawEnd, geneYLocation+7+2*ind*self.EACH_GENE_HEIGHT*zoom)),
                            ...
                            if (plotRight < (xLeftOffset + plotWidth - 3)) and (lastGene == 0):
                                ...
                                im_drawer.line(
                                    xy=((plotRight, geneYLocation+7+2*ind*self.EACH_GENE_HEIGHT*zoom),( drawEnd, geneYLocation+7+2*ind*self.EACH_GENE_HEIGHT*zoom)),
                                    fill= mylineColor, width=zoom*(self.EACH_GENE_HEIGHT+2))


                    if lastGene == 0:
                        im_drawer.text(
                            text="%s" % (self.genotype[0][i].name),
                            xy=(geneStartPix ,
...
## END HaplotypeAnalyst
    ...
    def drawClickBand(self, canvas, gifmap, plotXScale, offset= (40, 120, 80, 10), zoom = 1, startMb = None, endMb = None):
        ...
        if zoom == 1:
            for pixel in range(xLeftOffset, xLeftOffset + plotWidth, pixelStep):
                ...
                im_drawer.line(
                    xy=((xBrowse1, ucscPaddingTop),( xBrowse1, (ucscPaddingTop+self.BAND_HEIGHT))),
                ...
                im_drawer.line(
                    xy=((xBrowse1, ensemblPaddingTop),( xBrowse1, (ensemblPaddingTop+self.BAND_HEIGHT))),
            ...
            chrX = xLeftOffset + plotWidth - 2 - im_drawer.textsize("Chr %s" % self.ChrList[self.selectedChr][0], font=chrFont)[0]
            im_drawer.text(
            ...
            chrFont = ImageFont.truetype(font=VERDANA_FILE, size=26*zoom)
            traitFont = ImageFont.truetype(font=VERDANA_FILE, size=14)
            chrX = xLeftOffset + (plotWidth - im_drawer.textsize("Chr %s" % currentChromosome, font=chrFont)[0])/2
            im_drawer.text(
    ...
    def drawXAxis(self, canvas, drawAreaHeight, gifmap, plotXScale, showLocusForm, offset= (40, 120, 80, 10), zoom = 1, startMb = None, endMb = None):
        ...
        if self.plotScale == 'physic':
            ...
            im_drawer.text(
                text="Centimorgans",
                xy=(
                    xLeftOffset+(plotWidth-im_drawer.textsize("Megabases", font=centimorganLabelFont)[0])/2,
                    strYLoc + MBLabelFont.font.height+ 10*(zoom%2) + 10),
    ...
    def drawPermutationHistogram(self):
        ...
        #      Permutation Graph
        #########################################
        myCanvas = Image.new("RGBA", size=(400, 300))

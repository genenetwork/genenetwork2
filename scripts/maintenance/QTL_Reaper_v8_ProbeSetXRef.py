# Last Update by Zach on 8/11/2022 (adding conn.commit() for each loop)
...
#con = MySQLdb.Connect(db='db_webqtl',user='*',passwd='*', host="128.169.5.59")
#con = MySQLdb.Connect(db='db_webqtl_acenteno',user='webqtlupd',passwd='webqtl', host="localhost")
#conn = MySQLdb.Connect(db='db_webqtl',user='*',passwd='*', host="172.23.18.213")
#conn = MySQLdb.Connect(db='db_webqtl',user='*',passwd='*', host="tux04.genenetwork.org")
conn = MySQLdb.Connect(db='db_webqtl',user='*',passwd='*', host="tux02.genenetwork.org")
cursor = conn.cursor()
...
for ProbeSetFreezeId in ProbeSetFreezeIds:
	...
	for aProbeSetXRef in ProbeSetXRefInfos:
		...
		results = cursor.fetchall()
		if not results:
			print("CONTINUING FOR:", ProbeSetId)
			continue
		_strains = []
		_values = []
